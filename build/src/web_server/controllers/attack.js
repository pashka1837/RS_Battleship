...
import { createKillData, getHittedShip, isWinner, } from "../services/services.js";
export default function attack(data, isRandomAttack = false) {
    let { x: attackX, y: attackY, gameId, indexPlayer: curPlayerId } = data;
    const curGame = db.getGameById(gameId);
    if (curGame?.currentPlayerId !== curPlayerId)
        return true;
    if (isRandomAttack) {
        attackX = random(10);
        attackY = random(10);
    ...
    const enemyPlayer = [...curGame.players.values()].find((player) => player.playerId !== curPlayerId);
    const { hittedShip, removeI, deadShip, isHit } = getHittedShip(enemyPlayer.ships, attackX, attackY);
    if (deadShip) {
        attackX = -1;
        attackY = -1;
    }
