...
export function isWinner(enemyShips, curPlayerId, gameId) {
    if (enemyShips.some((ship) => ship.lifesLeft))
        return false;
...
export function getHittedShip(enemyShips, attackX, attackY) {
    let hittedShip = null;
    let removeI = null;
    let deadShip = false;
    let isHit = "miss";
    for (let z = 0; z < enemyShips.length; z++) {
        ...
        if (curShip.direction && enemyX !== attackX)
            continue;
        if (!curShip.direction && enemyY !== attackY)
            continue;
...
export function createKillData(hittedShip, curPlayerId) {
    const resData = [];
